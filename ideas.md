# JEC Network — direction de design

## Référence de vérité

Cette application doit reproduire le langage visuel et la structure fonctionnelle des maquettes fournies dans l’archive `stitch_jec_alumni_web_platform`. La fidélité aux références prévaut sur toute interprétation décorative.

### Header commun — référence « Accueil »

Le header desktop est une barre blanche fine, stable et nette : marque JEC Network à gauche, recherche centrale en forme de pilule gris bleuté, bouton noir « Publier », puis raccourcis notification, messagerie et avatar. Il doit rester disponible sur toutes les pages desktop et se simplifier, sur mobile, en barre avec menu, marque, recherche, notifications et avatar.
...
**JEC Network** est un réseau social professionnel destiné aux diplômés, qui transforme l’appartenance à une promotion en opportunités, entraide et rencontres concrètes.
...
- Les données mockées se rapprochent d’un réseau JEC crédible : promotions, campus, bureau, entreprises et identifiants professionnels cohérents.
